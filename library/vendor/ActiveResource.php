...
class ActiveResource {
	...
	function _xml_entities ($string) {
		if ('cdata:' === subStr($string, 0, 6)) {
			return '<![CDATA[' . subStr($string, 6) . ']]>';
		}
	...
	function _send_and_receive ($url, $method, $data = array ()) {
		...
		while (0 != preg_match('/^HTTP\/1\.\d/', $res)) {
	...
	function _fetch ($url, $method, $params) {
		...
		} elseif ('json' == $this->request_format) {
			curl_setopt ($ch, CURLOPT_HTTPHEADER, array ("Content-Type: text/json", "Length: " . strlen ($params)));
		}
		switch ($method) {
			case 'POST':
